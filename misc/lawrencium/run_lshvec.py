...
def get_model():
    ...
    model= LSHVec(model_file=os.path.join(lshvecdir , "model/genbank_model_gs_k23_l3000/model_299"), 
...
def get_clu_fastq_file(cid):
    ...
    if cid not in clu_fastq_files:
        fpath=LOCAL+"/{}/pacbio_cid_{}.fastq".format(basename, cid)
        f = open(fpath,'wb')
...
def handle_batch(batch):
    seqs=[]
    for a in batch:
        seqs.append(a[1][:-1].decode('ascii'))
