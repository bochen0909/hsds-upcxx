...
MPI_INPUT=${SANDBOXDIR}/kc_input
MIMIR_INPUT=${SANDBOXDIR}/kc_input
MRMPI_INPUT=${SANDBOXDIR}/kc_input
...
test_merge () {
  ...
  if [[ $1 == mimir ]]; then
  	merge_intrmed_mpi -n $NBUCKET -o $out3 --sep " " --sp 1 "${inputs[@]}"  > log3.txt 2>&1
  else
  	merge_intrmed_mpi -n $NBUCKET -o $out3 --sep "\t" --sp 1 "${inputs[@]}" > log3.txt 2>&1
  fi   
